...
Terminate an app by `identity` (exact pid, or a case-insensitive substring of the bundle id or app name — several matches return `ambiguous` and kill nothing). With no `signal`, escalates gracefully: SIGHUP → wait 2s → SIGTERM → wait 2s → SIGKILL, stopping as soon as it exits. With `signal` (SIGHUP/SIGINT/SIGTERM/SIGKILL or a number), sends only that one. No Accessibility required.
...
| `identity` | string | **yes** | Exact pid, or a case-insensitive substring of the bundle id or app name. Exact matches win; if more than one app matches, nothing is killed and the candidates are returned. |
