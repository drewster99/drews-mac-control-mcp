...
public enum SyntheticInput {
    ...
    public static func paste(_ text: String, consumed: (@Sendable () -> Bool)? = nil) -> PasteOutcome {
        ...
        let snapshot: [[NSPasteboard.PasteboardType: Data]] = (pasteboard.pasteboardItems ?? []).map { item in
            ...
            for type in item.types { types[type] = item.data(forType: type) ?? Data() }
            return types
        }
        ...
        guard pasteboard.setString(text, forType: .string) else {
            ...
            return outcome(posted: false, writeSnapshot() || writeSnapshot() ? .restored : .failed)
        }
        ...
        return outcome(posted: postedPaste, writeSnapshot() || writeSnapshot() ? .restored : .failed)
    }
}
